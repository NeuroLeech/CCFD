...
cortex = load_cortex("fsaverage6")          # this is `cortex`
...
max_len = 500    # Maximum boxcar length
...
A = np.c_[ signal,signal, signal,signal,signal*-1,signal*-1,signal*-1]
...
s = RotSWE(cortex.m, C/52.4*0, l=cortex.l, d=cortex.d, A=cortex.A,
           E=cortex.edges, bnd_edge=cortex.bnd)      # 52.4 = Ld, the frozen regime
s.set_sponge(sponge_profile(cortex.V, cortex.edges, cortex.bnd, 7.25, 0.27) + 0.003)
