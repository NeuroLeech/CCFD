...
one axis and can be swept together.
  ...
  python analysis/checkerboard_modulation.py --tag g7_s1.5_d25
"""
...
def piece_fields(c, tag, parcels, ecc=0, verbose=True):
    ...
    drive_P, ctr = P[sel], None
    if ecc:
        # the modulation's channels do not have to be the SOLVE's channels: the resting
        # background comes from the 47-piece solve either way, and the extra input is
        # free to have whatever spatial basis the question needs
        drive_P, ctr, _ = eccentricity_profiles(c, parcels[0], ecc, verbose=verbose)
        tags = [f"ecc{j}" for j in range(ecc)]
        sel = list(range(ecc))
    nfr = int(np.ceil(cm.RUN_S / frame_s))
    b = cm.boxcar(np.arange(nfr) * frame_s)
    imp = _impulse_frames(x, save, frame_s)
    R = xspec.impulse_responses(c, list(range(len(drive_P))), p, imp * save, save,
                                profiles=drive_P, verbose=False)
    kern = units.smoothing_kernel(timescale.bold_fwhm_frames(frame_s, verbose=False),
                                  verbose=False)
    Yb = np.zeros((len(drive_P), nfr, c.nV), np.float32)
    for j in range(len(drive_P)):
    ...
    return Yb, R, sel, tags, frame_drive(tag, save), save, kern, frame_s, ecc


def eccentricity_profiles(c, parcel=1, n=7, verbose=True):
    """Split a parcel into bands of constant ECCENTRICITY instead of by equal-area
    spectral bisection.

    `subparcels.split_parcels` cuts the mesh graph by its Fiedler vector, which balances
    area and follows geometry - and in V1 the axis it finds is not the eccentricity axis.
    The consequence is visible in the data: pieces 1_3, 1_4 and 1_5 all span roughly
    10-30 mm from the occipital pole, and 1_3 averages +0.019 where 1_5 averages +0.063,
    because each of them mixes foveal and peripheral vertices. Averaging inside such a
    piece destroys the one gradient the empirical response actually has (r = -0.75 against
    distance from the pole).

    Bands are cut at equal-AREA quantiles of that distance, so each carries the same
    cortical area the equal-area split gave its pieces, and the only thing that changes is
    which vertices are grouped. The profiles are tent functions on the band centres, which
    sum to exactly 1 across the parcel - a partition of unity, so the total drive injected
    is the same as a single whole-V1 profile and only its distribution moves.
    """
    import units
    inpar = np.flatnonzero(c.lab == parcel)
    pole = int(inpar[np.argmin(c.V[inpar, 1])])
    d = units.vertex_geodesic(c, [pole])[0]
    A = np.asarray(c.A, float)
    dv, av = d[inpar], A[inpar]

    o = np.argsort(dv)
    cum = np.cumsum(av[o]) / av.sum()
    edges = np.interp(np.linspace(0, 1, n + 1), cum, dv[o])
    ctr = 0.5 * (edges[:-1] + edges[1:])

    P = np.zeros((n, c.nV), np.float32)
    for j in range(n):
        lo = ctr[j - 1] if j > 0 else -1e9
        hi = ctr[j + 1] if j < n - 1 else 1e9
        left = np.clip((d[inpar] - lo) / max(ctr[j] - lo, 1e-9), 0, 1)
        right = np.clip((hi - d[inpar]) / max(hi - ctr[j], 1e-9), 0, 1)
        P[j, inpar] = np.minimum(left, right)
    s = P[:, inpar].sum(0)
    P[:, inpar] /= np.maximum(s, 1e-30)                # exact partition of unity
    if verbose:
        print(f"  {n} eccentricity bands of {parcel}, edges "
              + ", ".join(f"{v:.0f}" for v in edges) + " mm from the pole")
    return P, ctr, d
...
def run_sweep(c, t, tag, parcels, alphas, forms, lag_max, emp, d_drive, nwin=0,
              realise=0, seed=0, emp_piece=None, vprof=None, ecc=0, verbose=True):
    ...
    Yb, R, sel, tags, a_rest, save, kern, frame_s, ecc = piece_fields(
        c, tag, parcels, ecc=ecc)
    nfr = Yb.shape[1]
    if realise:
        F, k = None, realise
        ...
        if nwin:
            k = min(k, nwin)
    # With eccentricity bands the channels are not solve pieces, so there is no resting
    # drive of "band j" to scale against. The parcel's mean resting RMS is used instead,
    # which keeps alpha meaning the same fraction across every shape and split.
    v1p = [i for i in range(a_rest.shape[1])]
    rms = (np.full(len(sel), np.sqrt((a_rest ** 2).mean(0)).mean()) if ecc
           else np.sqrt((a_rest[:, sel] ** 2).mean(0)))
    ...
    for form in forms:
        for alpha in alphas:
            ...
            # the whole lag stack, not only the winner: the data's own maps are now
            # kept at every lag too, and travel can only be compared against travel
            out[nm] = dict(map=M, var=V, ag=ag, peaks=pk, lag=lag, lagmaps=A)
...
def main():
    ...
    ap.add_argument("--ecc", type=int, default=0, metavar="N",
                    help="split the driven parcel into N ECCENTRICITY bands - equal-area "
                         "quantiles of geodesic distance from the occipital pole - "
                         "instead of using the solve's own equal-area spectral pieces")
    ...
    if a.ecc:
        Pb, ctr, _ = eccentricity_profiles(c, parcels[0], a.ecc, verbose=False)
        wv = Pb[:, t.cols]
        ep = np.array([float(wv[j] @ emp / max(wv[j].sum(), 1e-30)) for j in range(a.ecc)])
    else:
    ...
    out, sel, tags, k = run_sweep(c, t, a.tag, parcels, alphas, forms, a.lag_max,
                                  ...
                                  seed=a.seed, emp_piece=ep, vprof=vp, ecc=a.ecc)
    ...
    np.savez(p, empirical=emp, cols=t.cols, d_drive=d_drive, nwin=k,
             ...
             **{f"lagmaps_{k2}": v["lagmaps"].astype(np.float32)
